...
public class Lazy_06_Terekhov {

    // From:
    // https://www.cs.umd.edu/~pugh/java/memoryModel/DoubleCheckedLocking.html#ThreadLocal

    static class TerekhovLazy<T> implements Lazy<T> {
        private final Box<Supplier<T>> factoryBox;
        private final ThreadLocal<String> threadLocal;
        private T value;

        public TerekhovLazy(final Supplier<T> factory) {
            this.factoryBox = new Box<>(factory);
        ...
        public T get() {
            if (threadLocal.get() == null) {
                synchronized(this) {
                    if (factoryBox.get() != null) {
                        value = factoryBox.get().get();
                        factoryBox.reset();
                    }
                }
                // NOTE: Original Terekhov's example sets threadLocal.set(threadLocal),
                // but that constructs a memory leak. As Tereknov's example correctly
                // notes, any non-null value would do as the argument here, so we just
                // put a String.
    ...
    public static class Basic {
        Lazy<Holder> lazy = new TerekhovLazy<>(() -> new Holder());
    ...
    public static class NullHolder {
        Lazy<Holder> lazy = new TerekhovLazy<>(() -> null);
        @Actor public void actor1(LL_Result r) { r.r1 = Lazy.poll(lazy); }
        @Actor public void actor2(LL_Result r) { r.r2 = Lazy.poll(lazy); }
    }
    ...
    public static class RacyPublication {
        Lazy<Holder> lazy;
        @Actor public void actor1() { lazy = new TerekhovLazy<>(() -> new Holder()); }
